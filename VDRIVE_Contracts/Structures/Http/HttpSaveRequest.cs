...
namespace VDRIVE_Contracts.Structures.Http
{
    ...
    public class HttpSaveRequest
    {
        ...
        // File data follows after the fixed structure
        public byte[] FileData;

        public static HttpSaveRequest ParseFromBytes(byte[] data)
        {
            ...
            var request = new HttpSaveRequest();
            ...
            // Copy file data (everything after session + length + filename)
            int fileDataOffset = 3 + filenameLength;
            int fileDataLength = data.Length - fileDataOffset;
            if (fileDataLength > 0)
            {
                request.FileData = new byte[fileDataLength];
                Array.Copy(data, fileDataOffset, request.FileData, 0, fileDataLength);
            }
            else
            {
                request.FileData = new byte[0];
            }
